...
class CrawlerObject(Spider):
    ...
    def init_func(cls):
        payload_list = (
            # 招标公告
            {
                "url": "https://www.zzsvtc.edu.cn/zbgg.htm",
                "page_number": 1,
            },
        )
    ...
    def get_list(self, params: dict):
        ...
        rows = soup.select('div#right > ul li')
    ...
    def get_content(self, params: dict):
        ...
        if script := content.select_one('script'):
            m = re.search(r'showVsbpdfIframe\("(.*?)",', script.text)
            if m:
                a_tag = soup.new_tag('a', href=m.group(1), string="内容附件")
