...
class CrawlerObject(Spider):
    ...
    def get_list(self, params: dict):
        ...
        rows = soup.select('table.news-table tr')

        for row in rows:
            a_tag = row.select_one('h1 a')
    ...
    def get_content(self, params: dict):
        ...
        soup = BeautifulSoup(resp.text, "html.parser")
        content = soup.select_one('div[style="text-align:center;"]')

        if content is None:
            content = soup.select_one('.main')

        if params['pubTime'] is None:
            t = soup.select_one('.main_time')
