...
class CrawlerObject(Spider):
    ...
    def init_func(cls):
        payload_list = (
            # 政务信息公开
            {
                "url": "http://www.heningjin.jcy.gov.cn/zwxxgk/index.shtml",
                "page_number": 1,
            },
        )
    ...
    def get_list(self, params: dict):
        ...
        rows = soup.select('td.bor-bx > table:nth-child(3) tr')
    ...
    def get_content(self, params: dict):
        ...
        m = re.search("var file_appendix='(.*?)'", resp.text)
        if m:
            content = f"{str(content)}\n{m.group(1)}"

        if params['pubTime'] is None:
            t = soup.select_one('td.b12[bgcolor="#F5F5F5"]')
