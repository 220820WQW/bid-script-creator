...
class CrawlerObject(Spider):
    ...
    def init_func(cls):
        payload_list = (
            # 公示公告
            {
                "url": "https://www.szfsyy.com.cn/admin.php/Homebase/getDocumentInfo",
                "page_number": 1,
                'data': {
                    "name": "公示公告",
                    "pageNo": "1",
    ...
    def get_list(self, params: dict):
        ...
        for row in rows:
            id = row.get('id')
            url = f"https://www.szfsyy.com.cn/pc/#/condition/detail?tab=YZDT&name=GSGF&id={id}"

            title = row.get('title')
            pubTime = row.get('newposttime')
