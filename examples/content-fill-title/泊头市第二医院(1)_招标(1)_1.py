...
class CrawlerObject(Spider):
    ...
    def init_func(cls):
        payload_list = (
            # 公告公示
            {
                "url": "http://botou.gov.cn/botou/c100829/listDisplaySelf.shtml",
                "page_number": 1,
            },
            # 政策文件
            {
                "url": "http://botou.gov.cn/botou/c100830/listDisplaySelf.shtml",
                "page_number": 1,
            },
        )
    ...
    def get_content(self, params: dict):
        ...
        if params['title'] is None:
            params['title'] = handle_str.replace_escape(
                soup.select_one('.detail h1').get_text(strip=True)
            ).strip()

        content = soup.select_one('div.articlecon')
