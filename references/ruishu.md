...
1. 使用本机可用的浏览器 MCP，并在访问目标栏目之前完整执行 `references/rs-browser-inject.md` 的预注入、创建新页面、导航和测试流程。只有该文档规定的检查通过后，才继续分析目标栏目；测试失败时按其“异常处理”完成排查。
2. 完成上述排查后，页面仍持续空白、停留在挑战页或无法正常加载栏目内容时，立即停止生成脚本，并说明失败步骤和已确认原因。
3. 页面能够正常加载时，继续按照 `SKILL.md` 和 `references/site-analysis.md` 完成栏目、列表、详情及附件分析；不得因直接请求返回 202 而停止。
...
禁止修改重试次数、网络请求、请求顺序、选择器、`general_cookie` 函数名、Cookie 合并及有效性判断结构；目标站点已验证的瑞数结构与固定模板不一致时，先向用户说明，禁止自行改造模板。
...
1. 真实参考案例统一从 `examples/ruishu/` 检索，只读取与目标请求方式和数据结构最接近的少量案例。
