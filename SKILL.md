...
根据用户提供的目标站点、栏目和采集页数，分析真实数据来源，最终生成完整、独立可运行的单文件 Python 采集脚本。采集字段包括 `title`、`url`、`pubTime`、`content`，及可下载附件。



## 最高优先级约束

1. 除了本次生成的文件，禁止修改任何无关文件。
2. 禁止运行或导入执行完整采集脚本。
3. 脚本中的网络请求默认必须使用 `auto_request`，包括列表、详情、正文接口、附件接口及辅助函数的发包请求；只有特殊案例的 reference 明确规定使用 `request.render_page`、`request` 或其他固定发包方式时，才按对应 reference 执行，禁止自行改换发包方式或使用 reference 未明确允许的其他网络库。
4. 生成脚本时必须保持简洁，只保留完成采集流程不可缺少的代码；能直接清楚表达的逻辑不增加无意义中间变量或封装，能复用现有值就不重复构造。代码排版、语义分组和必要中间变量必须遵守 `references/contract.md`，不得以减少代码行数为目的压缩代码。
...
7. 实际使用公共函数或辅助函数前，必须到 `references/framework-functions.md` 阅读对应函数小节。
8. 开始前必须先阅读 `references/contract.md` 熟悉脚本模板和框架契约。
...
必须完整遵守 `references/site-analysis.md`：
...
6. 命中瑞数、加速乐和阿里`acw_sc__v2`后必须按各自特殊 reference 执行。



### 3.生成脚本代码

1. 完成目标网页分析后、写入任何代码前，必须完整读取四个核心 reference：

   - `references/data-parsing.md`

   - `references/init-func.md`
   - `references/get-list.md`
   - `references/get-content.md`

2. 确认不存在会改变实现路径的未解决歧义。

3. 根据下表检索最接近的少量真实案例。

4. 严格根据分析的真实结果依次生成 `init_func`、`get_list`、`get_content`。
...
| 详情正文由 JavaScript 将接口 JSON 渲染为页面 DOM，静态 HTML 和接口响应均没有可直接返回的正文 HTML | `examples/render_content/` | `request.render_page`、渲染后 HTML、动态表格、正文 DOM、`sleep_time` |
...
1. `examples` 按脚本的主要特征分类。命中多个特征时可以依次检索多个目录，并优先检索对主请求、解析或特殊处理流程影响最大的特征目录；案例中可能同时包含其他特征，必须以其当前所在目录的主要特征为准。

2. 除瑞数、加速乐和阿里 `acw_sc__v2` 外，案例检索优先级为：

   - 已匹配上述案例特征时：当前匹配目录 -> `common-cases` -> `other-cases`。

   - 未匹配到案例特征时：`common-cases` -> `other-cases`。

3. 命中瑞数、加速乐或阿里 `acw_sc__v2` 时，固定优先检索各自特殊案例目录，不得改去通用加密目录套用其他流程。

4. 先用请求方式、分页方式、响应类型、字段路径、正文来源和特殊能力关键词缩小范围；每次只读取一个或少量最接近案例，禁止逐个读取整个目录。

5. 当前匹配的案例目录没有接近案例时，依次检索 `common-cases` -> `other-cases`。仍未找到时依据已验证事实和约束继续生成代码。

6. 案例只可参考方法契约、特殊场景写法。禁止复制案例中的站点参数、选择器、字段、失效参数、错误写法或无用代码。



## 交付前检查

1. 逐项核验脚本中的请求方式、请求参数、分页规律、字段路径、选择器、URL、正文和附件处理均有目标站点真实依据。
2. 按 `references/contract.md` 完成代码质量和固定契约检查。
3. 确认不存在会改变实现结果的未解决歧义；如仍有未确认事项，停止交付并先向用户说明和询问。
4. 使用 `ast.parse` 完成语法检查。
